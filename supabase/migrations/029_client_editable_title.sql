-- Letting the person who wrote a title fix it.
--
-- A request typed in a hurry keeps its typo: the title was not among the keys
-- a client may write, so the person who submitted it had to ask us to correct
-- their own words. `title` joins that list for work and requests. Everything
-- else here is 028 unchanged.
...
begin
  ...
  allowed := case TG_TABLE_NAME
    when 'work'     then array['comments','desc','approved','archived','title']
    when 'requests' then array['comments','files','links','desc','archived','title']
